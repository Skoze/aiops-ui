...
const Dashboard: FC<IDashboardProps> = props => {
  ...
  useEffect(() => {
    Promise.all([request.get('/metadata/getDatabases'), request.get('/metadata/getServices')])
      .then(res => {
        setDatabases(res[0]);
        setServices(res[1]);
        ...
        Promise.all([
          ...
          .then(response => {
            ...
            setSelectedIds({
              endpoint: res[0][0].serviceEndpointId,
              serviceInstance: res[1][0].instanceUuid,
            });
          })
          .catch(e => {
        ...
        <Tabs defaultActiveKey="1-1" onChange={onTabChange}>
          <Tabs.TabPane tab="Global" key="1-1">
            {filter.hasOwnProperty('type') && (
              <Global refresh={refresh} duration={duration} id={filter?.type} />
            )}
          </Tabs.TabPane>
          <Tabs.TabPane tab="Service" key="1-2">
            {filter.hasOwnProperty('service') && (
              <ServicePanel refresh={refresh} duration={duration} id={filter?.service} />
            )}
          </Tabs.TabPane>
          <Tabs.TabPane tab="Endpoint" key="1-3">
            {filter.hasOwnProperty('endpoint') && (
              <EndpointPanel refresh={refresh} duration={duration} id={filter?.endpoint} />
            )}
          </Tabs.TabPane>
          <Tabs.TabPane tab="Instance" key="1-4">
            {filter.hasOwnProperty('type') && (
              <InstancePanel
                ...
                instance={_.find(
                  serviceInstances,
                  (instance: ServiceInstancesType) => instance.instanceUuid === filter?.endpoint,
                )}
              />
            )}
          </Tabs.TabPane>
        ...
        <Tabs defaultActiveKey="1" onChange={onTabChange}>
          ...
          <Tabs.TabPane tab="Database" key="2-2">
            {filter.hasOwnProperty('database') && (
              <DatabasePanel refresh={refresh} duration={duration} id={filter?.database} />
            )}
          </Tabs.TabPane>
        </Tabs>
      )}
